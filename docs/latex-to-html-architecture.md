...
| RawHTML | `html` 环境或 `\importHTML` 命令，原样写入最终 HTML。 |
...
`\importHTML{...}` 是针对外部 HTML 片段的显式导入命令，由 block parser 处理，而不是 source loader 的通用 input/include 机制处理。parser 会按主文件目录解析相对路径，拒绝绝对路径和 `..` 逃逸路径；读取成功后生成 `ast.RawHTML`。文件不存在会记录 warning 并跳过该块；内容不像 HTML 时记录 warning，但仍保留原始内容，避免静默丢失用户输入。

...
my-blog/
  ...
  data/custom_components/     页尾和文章统计等自定义组件
  web/static/                 字体和静态资源
  .metablog-cache/            本地缓存
  out/                        默认构建输出
```
...
| Paragraph | 普通段落。 |
| MathBlock | `\[...\]`、`$$...$$`、`equation`、`align`、`aligned` 等。 |
| Figure | `figure` / `figure*`。 |
| Table | `table` / `table*`。 |
| List | `itemize`、`enumerate`、`description`。 |
| TCB | 自定义 `tcb` 环境。 |
| CodeBlock | `verbatim`、`lstlisting`、`minted`。 |
| RawHTML | `html` 环境、`\importHTML` 或 `\inputHTML` 命令，原样写入最终 HTML。 |
...
`html` 环境解析为 `ast.RawHTML`。parser 只剥离环境外壳，内部内容不进入 LaTeX block/inline parser；renderer 直接把该 HTML 片段写入文章正文，不做 HTML escape，也不做安全清洗。lexer 和 source loader 会把 `html` 作为原样环境保护，因此其中的 `%`、`\input`、`\include` 不参与注释剥离或文件展开。

这一点是 `html` 环境的关键边界：`\input{...}` 和 `\include{...}` 在 `html` 内部不会导入外部文件，而是作为原始 HTML 文本的一部分保留。该行为刻意对齐 `verbatim`、`lstlisting`、`minted` 等逐字类环境，避免 raw 内容中的 LaTeX-like 字符串被提前解释。

`\importHTML{...}` 是针对外部 HTML 片段的显式导入命令，由 block parser 处理，而不是 source loader 的通用 input/include 机制处理。`\inputHTML{...}` 是兼容别名，进入同一套解析流程。parser 会按主文件目录解析相对路径，拒绝绝对路径和 `..` 逃逸路径；读取成功后生成 `ast.RawHTML`。文件不存在会记录 warning 并跳过该块；内容不像 HTML 时记录 warning，但仍保留原始内容，避免静默丢失用户输入。

`html` 环境、`\importHTML{...}` 和兼容别名 `\inputHTML{...}` 也可以出现在普通段落内部。没有空白行切分时，parser 会生成 `ast.RawHTMLInline`，renderer 会把它原样写入当前 `<p>`。由于浏览器不允许 `<div>` 出现在 `<p>` 内部，inline raw HTML 中检测到 `<div>` 时会记录 warning；需要块级 HTML 时，应通过空白行把 raw HTML 与上下文分成独立块。
...
| --- | --- |
| 文本 | 普通文本。 |
| 样式 | `\textbf{...}`、`\textit{...}`、`\emph{...}`、`\texttt{...}`、`\textrm{...}`、`\textsf{...}`、`\textsc{...}`、`\textsl{...}`、`\textup{...}`、`\textmd{...}`、`\textnormal{...}`、`\underline{...}`。 |
...
`ast.Styled` 和 `ast.StyledBlock` 保存统一样式字段，包括颜色、背景色、字号、对齐、下划线、粗体、斜体、等宽字体、字体族、字形、字重和字形变体。命令式样式和声明式样式都会归一到这些字段，再由 renderer 统一生成 CSS。
...
| Inline | Text、Styled、Link、Reference、Citation、Math、LineBreak、RawHTMLInline。 |
...
5. `equation` / `equation*`
6. `align` / `align*`
7. `aligned` / `aligned*`、`alignedat` / `alignedat*`、`gathered`、`split` 和常见矩阵环境

当前不尝试手写解析数学内部语法。
Parser 只识别数学边界并生成 `DisplayMath` / `InlineMath` 节点，数学环境内部的 LaTeX 命令原样保留给 KaTeX，因此不会产生普通 inline command warning。
`\[...\]`、`$$...$$`、`equation*`、`align*` 和内部数学环境 fallback 会标记为不编号；`equation` / `align` 会进入公式编号序列。

Renderer 输出公式时会在 `.math.inline` 或 `.math.display` 节点上保留 `data-tex`。浏览器端脚本只对这些公式节点调用 KaTeX，不再扫描整个 `document.body`，也不再启用 `$...$` 自动分隔符。页面选中内容触发复制时，脚本会检查选区是否与公式节点相交；若包含公式，则把公式节点替换为 `data-tex` 中的 LaTeX 源码写入剪贴板纯文本。行内公式复制为 `\(...\)`，块级公式复制为 `\[...\]`。不包含公式的普通复制不会被拦截。

LaTeXML 复杂块清洗阶段也会把 MathML 按 `alttext` 还原为同一套 `.math.inline` / `.math.display` 结构，并写入 `data-tex`。因此复杂表格或算法块中的公式与 Go parser 直接解析出的公式共享同一套 KaTeX 渲染和复制源码逻辑。
...
`site serve -only-ram` 在服务启动后将输出文件操作移入内存。若同时启用 `-initial-build`，初始全量构建仍按普通 `site build` 写入 `out/` 一次，然后再加载到内存；后续 HTTP 服务和 watch 重建才完全基于 MemStore。

**MemStore**：`memStore` 是一个 `sync.RWMutex` 保护的内存文件表，以相对路径为键存储文件内容和内存中的修改时间。启动时通过 `loadDir` 递归加载整个 `out/` 目录。MemStore 实现了 `http.Handler` 接口，替代 `http.FileServer` 直接从内存响应用户请求。当前版本没有把初始全量构建的所有站点输出路径都改造成 MemStore 写入，因此 `-initial-build -only-ram` 会先写盘再读入内存，以保证首页、列表页、静态资源、文章页和 about 页都完整存在。
...
当前 `ast.Styled` 和 `ast.StyledBlock` 同时保留了两类样式字段：

1. 历史布尔字段：`Bold`、`Italic`、`Mono`、`Underline`。
2. 后续扩展的字符串字段：`FontFamily`、`FontStyle`、`FontWeight`、`FontVariant`、`FontSize`、`Color`、`Background`、`Align`。

这种双轨设计目前没有功能错误。`internal/render/html.go` 中的 `styleString` 会先读取字符串字段，再在字符串字段为空时从布尔字段推导 CSS，例如 `Italic=true` 会推导为 `font-style: italic`，而 `FontStyle="oblique"` 会优先使用显式字符串值。这样能兼容早期 inline/block 样式节点，也能支持 `\slshape`、`\scshape`、`\mdseries` 等更细的字体声明。

但该设计有维护成本：布尔字段中的 `false` 同时可能表示“未设置”和“显式恢复普通样式”，而字符串字段可以表达 `"normal"`、`"400"` 等显式恢复语义。随着字体族、字形、字重和变体继续增加，解析器和渲染器需要同时维护两套表达方式，容易出现某个命令只更新一条路径的问题。

后续重构目标是将样式状态收敛到单一结构，减少布尔/字符串混用：
...
1. 为 `ast.Styled` 和 `ast.StyledBlock` 引入统一的 `Style` 字段。
2. 将 `\textbf`、`\bfseries` 映射为 `FontWeight="700"`，将 `\textit`、`\itshape` 映射为 `FontStyle="italic"`。
3. 将 `\texttt`、`\ttfamily` 映射为 `FontFamily="mono"`，逐步移除 `Mono` 布尔字段。
4. 将 `\underline` 映射为 `TextDecoration="underline"`，逐步移除 `Underline` 布尔字段。
5. 在 renderer 中只消费统一 `Style`，不再从布尔字段推导 CSS。
6. 保留一轮兼容层或一次性迁移全部 parser 测试，避免旧 AST 构造测试静默失效。

该项暂不在当前修复中执行，原因是它会同时影响 AST、inline parser、block parser、renderer、测试和可能的 JSON 序列化输出。更合适的执行时机是一个独立的小版本重构，并配套覆盖所有字体命令、声明式样式、标题/caption/tcb 标题等文本参数入口的回归测试。

...
默认模板、SVG 和 `fonts.css` 都编译进二进制。字体文件从网络下载到 `web/static/fonts/`。Python、LaTeXML 和 PDF 转换器只做检测，失败时输出 `WARN` 和安装建议，不自动安装。
...
| `internal/site/builder.go` | 复制静态资源，写默认 CSS 和 warning 文件。 |
...
| `internal/render/*_test.go` | 编号、引用、HTML 输出。 |
...
2. 为 `lstlisting` / `minted` 加入语法高亮。
3. 增加环境检查命令，例如 `metablog doctor`。
4. 增加本地预览服务器。
5. 支持更多 bibliography 样式。
6. 支持可配置 KaTeX 资源来源。
7. 为 site init 的字体下载增加可配置镜像源。
